...
* WizTree 4.31 prefixes the export with a one-cell ``Generated by WizTree ...``
  note; leading banner rows are skipped before the header is read.
...
* ``Modified`` is ``yyyy/MM/dd HH:mm:ss`` or ``yyyy.MM.dd HH:mm:ss`` (the
  separator depended on the WizTree version), zone-less local time, stored as an
...
_WIZTREE_BANNER_RE = re.compile(r"^generated by wiztree\b")
"""WizTree 4.31's opening note (``Generated by WizTree 4.31 ...``)."""
...
class _Columns:
    ...
    width: int
    mapped_width: int
    """Cells through the last column the parser mapped.

    A row shorter than this is missing a column we know (counted as *short*);
    a row missing only trailing columns we never mapped -- WizTree 4.31 puts
    drive-summary columns after ``Folders`` and fills them on the root row
    alone -- is not.
    """
...
def _parse_header(header: list[str]) -> _Header:
    ...
    columns = _Columns(
        width=len(header),
        mapped_width=max(seen.values()) + 1,
        path=seen["path"],
        size=seen["size"],
...
class _Loader:
    ...
    def handle(self, row: list[str]) -> None:
        ...
        if not row or not any(cell.strip() for cell in row):
            ...
            return
        if len(row) < self._columns.mapped_width:
...
def _is_banner_row(row: list[str]) -> bool:
    """True for WizTree's leading ``Generated by WizTree ...`` note (4.31+)."""
    return bool(row) and bool(_WIZTREE_BANNER_RE.match(_normalise_header_cell(row[0])))


def _read_header(reader: Iterator[list[str]]) -> list[str]:
    """Return the header row, skipping any leading WizTree banner note."""
    for row in reader:
