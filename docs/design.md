...
- **Banner.** WizTree 4.31 prefixes the file with a one-cell
  `Generated by WizTree ...` note; leading banner rows are skipped before the
  header is read (older versions start with the header itself), and the banner
  is not an entry.
...
- **Storage normalization.** Folder paths are stored without the trailing
  ...
  exactly as exported.
- **Timestamps.** `Modified` is `yyyy/MM/dd HH:mm:ss` or `yyyy.MM.dd HH:mm:ss`
  (WizTree switched separators between versions), zone-less local time.
...
- **Tolerances (all counted, never fatal).** blank lines, short rows (fewer
  cells than through the last column the parser mapped; missing cells become
  `""`/`NULL`), long rows (extra cells ignored), empty name cells,
...
- **Every long line is elided *or* wrapped on purpose — never clipped.** The plan's meta line and the status bar carry paths, so `widgets.ElidedLabel` elides them (middle for a path, right for a sentence) instead of letting a `QLabel` paint past its edge; the *What happens* column is a sentence, so `plan_models.PlanDetailDelegate` cuts it at whole words (`detail_line`) where the default middle-elision produced "moves to /var/tmp/space…Mode)". The tooltips keep the full text in both cases. Neither widget paints a cut glyph, and both say which text they were given (`full_text()`): `ElidedLabel(..., claim_width=True)` and `widgets.ElidedButton` *claim* the width their whole text needs (the figure the user confirms, the caption of the action itself), so a short row shrinks them last; the labels that only annotate — a path, a hint, a sentence — ask for nothing and yield first. `widgets.caption_room()` measures how much width a button's style leaves for its caption. When a row cannot pay at all, it *wraps*: the six action bars (the Plan toolbar asks for 1192px, the Undo footer 949px, the Opportunities strip's six cards 837px, the filter bar's search was down to 46px) are `widgets.FlowLayout`, so the shell's 980x620 window puts their last items on a second line and every figure, caption and card title stays whole — `tests/gui/test_min_size.py` walks all six at 980x620 *and* at 1440x900, where each is back on one line. Elision is what is left for text no line can hold (a path, a long hint), with the ellipsis visible and the tooltip leading with the whole of it. `FlowLayout.addWidget(widget, stretch)` gives a line's leftover width to the items that ask for it, which is how the filter bar's search keeps its 490px at the reference size while the bar still wraps at the minimum.
...
### 9.4 AI surfaces (S10b findings)

- **A row of badges or buttons wraps; it never clips.** Qt has no wrapping layout, and a `QHBoxLayout` silently paints its last widget past the edge of a resizable pane: the AI card's four actions ("Suggest with AI", "Classify with AI", "Explain with AI", "Apply as rule…") overflowed the details pane's narrow width with no scrollbar to reveal them. `widgets.FlowLayout` carries the card's verdict badges, its actions, the header chips and the metric strip, so a narrow pane grows taller instead of hiding controls.
- **The pane scrolls to the answer it was asked for.** The AI cards sit below the reasoning, so a fresh suggestion or explanation rendered below the fold — the user pressed a button and saw nothing change. A request reveals its card *before* the card is rebuilt: a rebuilt card has no geometry until the next layout pass, and measuring it scrolls to the pane's top instead. `request_row`/`begin_explanation`/`set_ai_busy` all reveal the card in the slot the answer will occupy.
- **An empty state is a sentence, not a gap.** An explanation card with only its heading reads as a rendering fault; it says what would appear there and which button produces it ("Nothing yet: Explain with AI asks the provider what this row is…"). The card body stays the one live surface a streamed answer writes into.
- **`NO_ACTION` is an answer, not a task.** The column renders it as "No action" painted muted (`models.solution_display`) instead of a tier colour, because a row the AI refuses to act on is the honest outcome the rules cannot express, and painting it like a suggestion would invite an executable reading.
- **AI output is advisory by construction, and the tests prove it.** A suggestion or classification becomes a *proposal*: promoting one writes a user rule that the deterministic engine then applies, and "Apply as rule" is refused for tiers the executor may not touch (T1/T2 move-and-delete). A plan's action list is only ever written by the rule engine — the AI can annotate it, never extend it.
- **One handler owns the switches that decide what leaves the machine.** Provider, redact-paths, local-only and streaming all persist through `_on_policy`, so a settings screen can never save a half-applied privacy stance; the streaming toggle is a third member of that row rather than a separate provider field.
