...
def test_wiztree_431_banner_and_dotted_timestamps(tmp_path: Path) -> None:
    """WizTree 4.31 leads with a ``Generated by`` note and stamps dates with dots.

    The version line is not a header and not an entry; the ``Modified`` cells it
    exports (``yyyy.MM.dd HH:mm:ss``) are the same zone-less local times under a
    different separator, so they must parse like the slash form.
    """
    csv = write_csv(
        tmp_path,
        "Generated by WizTree 4.31 23.9.2026. 10:50:00 "
        "(You can hide this message by making a donation)\n"
        "File Name,Size,Allocated,Modified,Attributes,Files,Folders,"
        "DRIVECAPACITY,FREESPACE,USEDSPACE,RESERVEDSPACE\n"
        '"C:\\",1500,2048,2026.09.22 20:17:51,6,2,0,244511141888,9653133312,'
        "234858008576,5424119808\n"
        '"C:\\a.bin",1000,1024,2026.09.01 08:00:00,32,0,0\n'
        '"C:\\b.bin",500,1024,2026.08.31 23:59:59,32,0,0\n',
        name="wiztree431.csv",
    )
    db_path = tmp_path / "index.db"
    stats = ingest_csv(csv, db_path)

    assert stats.header[0] == "File Name"
    assert stats.rows == 3  # the banner row is neither a header nor an entry
    assert stats.bad_mtime_rows == 0
    # The drive-summary columns only the root row fills must not read as short rows.
    assert stats.short_rows == 0
    rows = {row["path"]: row for row in fetch_rows(db_path)}
    assert rows["C:\\"]["size"] == 1_500
    assert rows["C:\\a.bin"]["mtime"] == int(datetime(2026, 9, 1, 8, 0, 0, tzinfo=UTC).timestamp())
    assert rows["C:\\b.bin"]["mtime"] == int(
        datetime(2026, 8, 31, 23, 59, 59, tzinfo=UTC).timestamp()
    )
...
ROUND_TRIP_VARIANTS: dict[str, gen.GenOptions] = {
    ...
    "wiztree-431": gen.GenOptions(seed=7, min_files=150, banner=True, date_separator="."),
}
